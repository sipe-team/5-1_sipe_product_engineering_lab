...
- 고객의 수많은 리뷰를 다 읽어볼 수가 없기에, AI 에게 리뷰 요약을 시기고 사람은 그 결과를 해석하고 판단하는 형태로 바뀌었다.
- 리뷰를 분석할 때, 메타 정보를 분석 하고, 고객 세그먼트 별 니즈 파악, 정책 개선까지 갈 수 있도록 인사이트를 확장할 수 있다.
- 실무 팁을 읽으며, 문득 고등학교때 수능 준비를 하며 영어 공부를 했던 기억이 떠올랐다. 그때 주요하게 보라고 했던 게,
- however 같은 번전의 의미를 가지는 접속사, better than 같은 비교급 같은 단어 였는데. 문득 그때가 떠올라서 어떤 문장이건 이런 키워드가 중요하는 구나 생각이 나서 미소가 새어나왔다.
- 또한, 리뷰를 통해 사용자 시나리오로 연결하고, 이를 신제품 기획으로 가지를 뻗어나가는 점도 큰 인사이트로 느껴졌다.

글 2: [딜라이트룸 - 사용자 피드백은 제품 개선에 도움이 될까?](https://medium.com/delightroom/%EC%82%AC%EC%9A%A9%EC%9E%90-%ED%94%BC%EB%93%9C%EB%B0%B1%EC%9D%80-%EC%A0%9C%ED%92%88-%EA%B0%9C%EC%84%A0%EC%97%90-%EB%8F%84%EC%9B%80%EC%9D%B4-%EB%90%A0%EA%B9%8C-95cd98438ba0)
- 딜라이트룸의 CS 팀이 하는 일에 대한 소개가 주요 내용이었는데, 일반적인 회사의 CS팀보다는 조금 더 상위 레벨에서 업무를 진행한다는 점이 특색있게 느껴졌다.
- 특히, 사용자 입장을 공감하는 과정에서, 이 피드백이 정규 분포상의 어디쯤에 위치하는 의견일 지 고민한다는 점이 좋았다.
- 전 회사 대표님은 정말 마이너한 의견까지 반영해 버리는 바람에, 몇몇 소수가 좋아하는 것을 위해 많은 시간과 비용을 쏟았으며, 그런 직관에 의한 결정 떄문에 쓴 실패를 여러번 맛보았다.
- 뭔가 더 구체적인 내용이 궁금했지만, 아무래도 회사의 내부 정책이나 시스템에 대한 것이라 깊게 이해한 느낌은 들지 않아 아쉬웠다.
- 그럼에도, 상자 밖에서 그 문제의 관점을 다시 보라는 인사이트는 이 글에서 제일 좋게 느껴졌던 포인트였다.

글 3: [VoC를 체계적으로 관리하고 제대로 이해하는 법: VoC 프로그램](https://www.syncly.kr/blog/voc-program)
- VoC 프로그램을 운영하고 있는 기업은, 수익에서는 10%, 고객 리텐션 측면에서는 55% 나 높은 수지를 보였다고 한다.
- 향상된 고객 만족도는 레퍼럴에도 큰 효과를 주고, 신규 회원을 유치하기에도 긍정적인 영향을 줄 수 있다.
- 고객 입장에서도 불만을 말하고, 문의를 접수한다는 것이 큰 노동이기 때문에, 그에 따른 적절한 솔루션이나 대응을 받지 못한다면 이탈이 생길 수 밖에 없고.
- 나아가, 심각한 시스템 결함을 놓칠 수도 있을 것이라 생각한다.
- 운 좋게, 나는 VoC 시스템이 잘 갖춰진 회사에서 근무해봤는데. VoC 를 적극적으로 대응하며 고객의 만족도가 꾸준히 올라가는 것을 지표로 보니 뭔지 모를 뿌듯함을 느꼈던 기억이 있다.