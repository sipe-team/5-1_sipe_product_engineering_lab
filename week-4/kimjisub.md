...
**영향받는 사용자 규모 (추정)**: 채용 페이지 공시 기준 누적 유료 구매 3,795명 중, 월평균 2.52회에 닿지 못하는 비율을 고인물 편중을 고려해 보수적으로 잡으면 **유료 전환자의 30~50%**가 이 문제 세그먼트. 12개월 신청자 17,000명까지 확장하면 잠재 영향권은 더 넓다.
...
| ID | 기능 | 한줄 요약 | 상세 문서 |
|----|------|---------|---------|
| F1 | `is_newbie_welcome` 필드 + 호스트 체크박스 UI | 모임 생성 시 호스트가 뉴비 트랙을 약속할 수 있는 최소 장치 | TBD |
| F2 | 가입 60일 이내 전용 뉴비 캐러셀 | 신규에게만 보이는 별도 홈 상단 컴포넌트 | TBD |
| F3 | 모임 확정 시 뉴비 비율 자동 체크 | 50% 미달 시 태그 자동 해제하는 시스템 규칙 | TBD |
| F4 | 30일 첫 참여율 + 지니계수 대시보드 | MVP 판단 근거 지표 자동 집계 | TBD |
