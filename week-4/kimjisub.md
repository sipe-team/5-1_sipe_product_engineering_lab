...
| 단계 | 측정 기간 | 규모 | 출처/추정 근거 |
|------|---------|-----|-------------|
| 인터뷰 신청 | 12개월 플로우 | 17,000 | 채용 페이지 공시 |
| 유료 전환 | 누적 스톡 | 3,795 | 채용 페이지 공시 |
| 월평균 활성 참여자 | 최근 월 플로우 | **~559~1,117** (감도 의존) | 진행된 동행 4,222회 ÷ 12개월 × 모임당 N명 ÷ 2.52회/월 |
| 고빈도 코어 | 누적 스톡 | 소수 | 1인 최다 132회 누적 (≈ 전체 4,222회의 3.1%) |

<details>
<summary><b>📐 모임당 평균 참가자 수(N) 가정에 따른 감도 분석 + 해석 주의 (펼쳐보기)</b></summary>

**산출 공식**: 월 활성 참여자 ≈ (진행된 동행 4,222회 ÷ 12개월) × N명 ÷ 2.52회/월

| N 가정 | 월 활성 참여자 추정 | "스톡-플로우 격차"(3,795 − 월활성) |
|-------|-------------------|--------------------------------|
| 4명 | ~559 | ~3,236 (85%) |
| **6명** (중앙 가정) | **~838** | **~2,957 (78%)** |
| 8명 | ~1,117 | ~2,678 (71%) |
...
- 즉 이 78%는 "타겟 세그먼트 크기의 상한 추정"으로 읽어야 하며, 정확한 핵심 타겟은 **"결제 유지 중이면서 최근 월 미참여인 유료자"**다. 이 정제된 수치는 baseline 확보 시 산출 가능 (Open Questions #1, #7, #8).
- 감도표에서 보듯 N=6 가정의 40% 변동 범위(559~1,117)만으로도 세그먼트 크기가 크게 흔들리므로, MVP 설계 진입 전 **모임당 평균 참가자 수**를 확인하는 것이 선행 필수.
