...
**동행은 인터뷰를 통해 "들어오는 사람"은 걸렀지만, "들어온 이후 누가 누구와 어울리는가"는 걸러내지 못했다.**

3주차 공개 데이터 분석에서 확인된 핵심 수치:
- 월평균 참여 **2.52회** vs 1인 최다 참여 **132회** (채용 페이지 자체 공시) — 50배 이상의 참여 격차
- 내부 만족도 **4.68/5.0** (10,132건) vs 앱스토어 평점 4.2~4.4 — 체감 격차
- App Store 평점 분포의 양극화: 5점 71개 / 4점 **0개** / 1점 16개 — 경험이 "매우 좋거나 매우 나쁘거나"로 갈림
...
| 단계 | 규모 | 직전 단계 대비 전환 | 출처/추정 근거 |
|------|-----|-------------------|-------------|
| 인터뷰 신청 (12개월) | 17,000 | — | 채용 페이지 공시 |
| 유료 전환 (누적) | 3,795 | **22%** | 채용 페이지 공시 |
| 월평균 2.52회+ 활성 참여자 | ~838 (추정) | **~22%** | 진행된 동행 4,222회 × 모임당 평균 6명(가정) ÷ 2.52회/월 ÷ 12개월 |
| 고빈도 코어 (월 10회+) | 소수 | — | 1인 최다 132회 누적 |

**이 퍼널의 의미**: 인터뷰 통과 후 유료 전환된 **3,795명 중 78%(약 2,957명)가 활성 참여자 세그먼트에 도달하지 못하는 것**으로 추정. 이 "잠자는 유료자 2,957명"이 본 PRD의 핵심 타겟 세그먼트다. (단 모임당 평균 참가자 수 가정에 의존하는 추정치이므로 Open Questions 1번의 baseline 확보가 선행되어야 함)
...
**2) Revenue 상한이 구조적으로 제한된다**

15만원/3개월 구독은 "이번 분기 동안 몇 번 참여했는가"와 강하게 연동된다. 신규가 참여를 시작 못 하면 갱신도 없다.

> → 인터뷰 통과 신규가 늘어도 참여자 풀은 그만큼 늘지 않는다.
>
> → 마케팅으로 top of funnel을 넓혀도 bottom이 새는 구조가 고착된다.
...
한 사람이 132회 참여하는 구조는 당장은 커뮤니티가 "살아있어 보이게" 하지만, 같은 사람끼리의 반복 모임은 서서히 피로해진다.
...
| Retention | 신규 90일 리텐션 | 가입 90일차 월 1회+ 참여율 | 미측정 | V1 이후 평가 **+10%p** |
...
1. **참여 횟수 상한(쿨다운)**: 코어의 월 참여 횟수를 제한해 슬롯을 강제 확보. → 거절. 월 132회 참여하는 코어는 이 서비스의 가장 강한 팬이자 "살아있는 생태계"의 유일한 증거다. 제재는 즉각적 이탈 리스크를 만든다.
...
|-----------|------|-------:|-----------:|-----:|----------:|------|
| **A. 뉴비환영 태그 + 신규 큐레이션 피드** | 호스트 자발적 태그로 신규 전용 진입 경로 생성 | 7 | 8 | 9 | **504** | **채택** |
...
- **A-Ease 9**: 불리언 필드 1개, 홈 피드 분기 렌더링, 이행률 자동 체크 규칙만으로 MVP 성립. 2주 개발 가능.
...
| F4 | 30일 첫 참여율 + 지니계수 대시보드 (**이벤트 로그 기반**) | `newbie_carousel_impression` / `newbie_meeting_apply` / `first_meeting_complete` / `second_meeting_apply` 4개 이벤트 집계 | TBD |
...
| # | 질문 | 왜 중요한가 | 답을 찾는 방법 |
|---|------|----------|-------------|
| 1 | 현재 신규 30일 첫 모임 참여율의 실제 baseline은? | 목표 +15%p가 타당한지, MVP 성공 판단 기준을 만들 수 있는지 좌우 | MVP 설계 진입 전 내부 데이터 쿼리 (선행 필수) |
...
**대응**:
- 운영진이 MVP 초기 2주간 **뉴비환영 모임을 직접 기획해 기본 공급**을 채운다 (V1의 F7을 MVP에 선행 투입)
- 태그 부착 호스트에게 인센티브 (이용료 일부 크레딧, 호스트 리더보드 가산점)
- 관측 목표에 "호스트 자발 태그율"을 추가해 수동 공급과 자발 공급 비중을 분리 측정
